...
## Local DNS Architecture & Split DNS over Tailscale

**[Date: 08-09-2026]**

### Problem & Context

- Accessing network infrastructure (MikroTik WebFig, TP-Link switch, Proxmox VE, and Windows Server VM) required remembering raw IP addresses (`10.0.0.1`, `10.0.0.2`, `10.0.0.20`, `10.0.0.30`).
- Proxying embedded appliances (MikroTik and TP-Link Switch) through Nginx Proxy Manager (NPM) on port 80 broke session headers and web socket handshakes.
- In MikroTik RouterOS 7, a wildcard domain entry (`olympus-luca.online match-subdomain=yes` or `home.olympus-luca.online match-subdomain=yes`) acted as a global zone override, hijacking all subdomain DNS requests and sending them directly to Node 01 (`10.0.0.10`).

### Resolution

1.  **Removed Wildcard Interception:**
    - Purged `match-subdomain=yes` and wildcard regex rules from MikroTik static DNS.
...
2.  **Configured Explicit Static DNS Records on MikroTik:**
    ...
    /ip dns static add name="nd.home.olympus-luca.online" address=10.0.0.10
    /ip dns static add name="nc-aio.home.olympus-luca.online" address=10.0.0.10
    /ip dns cache flush
    ```

3.  **Tailscale Remote Access Integration:**
    - Leveraged Node 01's existing Tailscale Subnet Router (`10.0.0.0/24`).
    - Configured Tailscale Admin DNS Split DNS:
      - **Nameserver**: `10.0.0.1` (MikroTik Gateway)
      - **Search Domain**: `home.olympus-luca.online`
    - Guarantees seamless resolution and reachability of all homelab appliances and servers when operating outside the local network.
