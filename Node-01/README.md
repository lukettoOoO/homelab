_This is the first device that I'm adding to my homelab setup, it will run a Debian 13 server permanently on._
...
_• `/sys/class/scsi_host/host_/scan`: The /sys (sysfs) virtual filesystem exposes direct kernel interfaces for hardware drivers. Every SATA controller port, AHCI channel, or USB-to-SATA bridge registers a SCSI host directory (host0, host1, host2, etc.) containing a virtual file named scan.*
...
- Selected an _Axagon EE35-XA3_ aluminum USB 3.0 enclosure with a dedicated mechanical 2-position rocker switch (`I / O`). Leaving the hardware switch in the `I (ON)` position guarantees automatic drive spin-up directly on power restoration.
...
## Homelab Network DNS Expansion & Dashboard Webpage Update

**Date: 2026-09-08**

### 1. Networking Infrastructure Domain Integration

- Network devices (MikroTik RouterOS `10.0.0.1`, TP-Link switch `10.0.0.2`, Proxmox VE `10.0.0.20`, and Windows Server Dionysus `10.0.0.30`) lacked clean domain names and were accessed only via raw IP addresses.
- Proxying MikroTik WebFig and the TP-Link Switch through Nginx Proxy Manager (NPM) on port 80 failed because their embedded web servers rejected proxied headers, connection rewriting, and WebSocket handshakes.
- In MikroTik RouterOS 7, a wildcard entry (`match-subdomain=yes`) overrode exact static DNS records, redirecting every subdomain lookup directly to Node 01 (`10.0.0.10`). Furthermore, Cloudflare public DNS (`1.1.1.1`) had a public wildcard answering `10.0.0.10`.

- Removed wildcard and regex entries that intercepted DNS lookups.
...
- Identified protocol constraints: network appliances (MikroTik and TP-Link Switch) serve over plain **HTTP** (port 80) rather than HTTPS.

### 2. Updating Olympus Homelab Dashboard
